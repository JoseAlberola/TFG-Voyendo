...
public interface CompanyRepository extends CrudRepository<Company, Long> {
    ...
    @Query(nativeQuery = true, value = "SELECT c.* FROM COMPANIES c INNER JOIN (SELECT * FROM LABOURS " +
            "WHERE LOWER(name) LIKE LOWER(?1)) l ON c.id = l.company_id GROUP BY c.id")
